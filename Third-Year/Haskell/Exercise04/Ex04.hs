...
ins k d (Leaf ke de) 
  | k < ke = Branch (Leaf k d) Empty ke de
  | k > ke = Branch Empty (Leaf k d) ke de
  | otherwise = Leaf k d

ins k d (Branch left right ke de)
  | k < ke = Branch (ins k d left) right ke de 
  | k > ke = Branch left (ins k d right) ke de
...
lkp (Leaf k d) ke
  |ke == k = return d
  |otherwise = fail "No Key"
lkp (Branch left right k d) ke
  |ke == k = return d
  |ke < k = lkp left ke
  |ke > k = lkp right ke
  |otherwise = fail "No Key"
